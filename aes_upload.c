...
// Inisialisasi IP dan port tujuan TCP/IP.
TCPClient client;

IPAddress tcpIP(192,168,43,113);
int tcpPort = 5050;

// Inisialisasi IP dan port tujuan UDP.
UDP Udp;

IPAddress remoteIP(192,168,43,113);
int udpPort = 1337;

// Waktu awal eksekusi program.
...
void setup() {
  ...
  // Fungsi koneksi via TCP/IP.
  client.connect(tcpIP, tcpPort);
  // Digunakan untuk koneksi UDP dua arah.
  Udp.begin(8888);
}
...
// Fungsi AddRoundKey() untuk memasukkan kunci pada state.
...
void loop()
{
  ...
  // Untuk pembacaan tekanan udara dan ketinggian.
  ...
  // Konversi data sensor (string) menjadi hexadesimal.
  ...
  // Waktu seluruh eksekusi fungsi.
  unsigned long current = micros();
  Serial.printf("Time: ");
  unsigned long delta = current-start;

  // Waktu interval (delta) eksekusi.
  Serial.println(delta);
  Serial.printlnf("\n\n");

  // Mengirim paket via TCP/IP.
  if (client.connected())
  {
    client.write(enc);
  }

  // Mengirim paket via UDP.
  if (Udp.sendPacket(enc, sizeof(enc), remoteIP, udpPort) < 0)
  {
    Serial.printlnf("Error");
  }

  // Delay untuk proses eksekusi program pada fungsi loop().
  delay(3000);
}
